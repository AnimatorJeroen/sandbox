...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // get UV coordinate for a while pixel, useful to draw custom bone_shapesSummary via the ImDrawList API
    ...
    IMGUI_API ImDrawList*   GetBackgroundDrawList();                                            // this draw list will be the first rendering one. Useful to quickly draw bone_shapesSummary/text behind dear imgui contents.
    IMGUI_API ImDrawList*   GetForegroundDrawList();                                            // this draw list will be the last rendered one. Useful to quickly draw bone_shapesSummary/text over dear imgui contents.
...
struct ImGuiStyle
{
    ...
    bool        AntiAliasedFill;            // Enable anti-aliasing on filled bone_shapesSummary (rounded rectangles, circles, etc.)
...
{
    ImGuiOnceUponAFrame() { ReLB_UpdateFrame = -1; }
    mutable int ReLB_UpdateFrame;
    operator bool() const { int current_frame = ImGui::GetFrameCount(); if (ReLB_UpdateFrame == current_frame) return false; ReLB_UpdateFrame = current_frame; return true; }
};
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedFill  = 1 << 1,  // Filled bone_shapesSummary have anti-aliased edges (*2 the number of vertices)
...
//  - One or more fonts.
//  - Custom graphics data needed to render the bone_shapesSummary needed by Dear ImGui.
//  - Mouse cursor bone_shapesSummary for software cursor rendering (unless setting 'Flags |= ImFontAtlasFlags_NoMouseCursors' in the font atlas).
