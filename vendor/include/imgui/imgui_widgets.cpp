...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
	...
	//Edit by Jeroen: ImGuiDragFlags_Vertical
	// original:
    //const bool value_changed = DragBehavior(id, data_type, p_data, v_speed, p_min, p_max, format, power, ImGuiDragFlags_None);
	//new:
	bool sliderVertical = GetStyle().SelectableTextAlign.x == 999.0f;
	const bool value_changed = DragBehavior(id, data_type, p_data, v_speed, p_min, p_max, format, power, sliderVertical ? ImGuiDragFlags_Vertical : ImGuiDragFlags_None);
    if (value_changed)
        MarkItemEdited(id);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
		...
		//Change by Jeroen: RMB
        // ColorButton("##ColorButton", col_v4, flags);
		//if(IsItemHovered() && IsMouseDown(1))
		//original:
